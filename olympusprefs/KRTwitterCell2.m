...
//
// Apache 2.0 License for code used in KRPrefsLicense located in preference
// bundle
//

#import "KRTwitterCell2.h"
...
@interface KRTwitterCell2 () {
  NSString *_user;
}

@end

@implementation KRTwitterCell2
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
  ...
  if (self) {
    ...
    imageView.image =
        nil; //[UIImage imageNamed:@"twitter" inBundle:globalBundle];
    [imageView sizeToFit];
...
- (void)loadAvatarIfNeeded {
  ...
  // TODO: fix this
  // self.avatarImage = [UIImage imageNamed:[NSString
  // stringWithFormat:@"/Library/PreferenceBundles/SignePrefs.bundle/%@.png",
  // _user]];

  dispatch_async(dispatch_get_global_queue(0, 0), ^{
    // NSData * data = [[NSData alloc] initWithContentsOfURL:[NSURL
    // URLWithString:[NSString
    // stringWithFormat:@"https://twitter.com/%@/profile_image?size=original",
    // _user]]];
    NSData *data = [[NSData alloc]
        initWithContentsOfURL:
            [NSURL URLWithString:@"https://pbs.twimg.com/profile_images/"
                                 @"1434977707473969158/lvWjkp6p_400x400.jpg"]];
    if (data == nil)
      return;
